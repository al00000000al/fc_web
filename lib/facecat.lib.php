...
function FC_AuthDirect(){
    $secret = "wUNxZctxj3HTj7RP0zTVKPH5uTZWjcM8ss3F8B7XPmFWsAwNVB0EjdCudTsUTcy6UzRvPjzREtQmBYFvFxk4QBL5vfmh7mmeb52F";
    $nounce = round(microtime(true) * 1000)."";
    $sign = md5($nounce.$secret);
    $ch = curl_init();

    curl_setopt($ch, CURLOPT_URL, 'https://face.cat/api/auth/direct');
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
    curl_setopt($ch, CURLOPT_POSTFIELDS, json_encode(['nonce' => $nounce, 'sign' =>$sign]));
    curl_setopt($ch, CURLOPT_POST, 1);
...
function FC_ApiSend($chat_id, $token, $msg, $cat = 0){
    // Generated by curl-to-PHP: http://incarnate.github.io/curl-to-php/
    $ch = curl_init();
$random = mt_rand(11111111,999999999999);
    curl_setopt($ch, CURLOPT_URL, 'https://face.cat/api/chat/'.$chat_id.'/message?random_id='.$random.'&cat='.$cat);
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
...
function FC_ApiUserIdGet($user_id, $token){

    $ch = curl_init();

    curl_setopt($ch, CURLOPT_URL, "https://face.cat/api/user/{$user_id}");
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
    curl_setopt($ch, CURLOPT_CUSTOMREQUEST, 'GET');
...
function FC_ApiChatUrlCodeGet($code, $token){
    $ch = curl_init();

    curl_setopt($ch, CURLOPT_URL, "https://face.cat/api/chat/url/{$code}");
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
    curl_setopt($ch, CURLOPT_CUSTOMREQUEST, 'GET');
...
function FC_ApiContactPost($user_id, $token){
    // Generated by curl-to-PHP: http://incarnate.github.io/curl-to-php/
    $ch = curl_init();
    $random = mt_rand(11111111,999999999999);
    curl_setopt($ch, CURLOPT_URL, 'https://face.cat/contact');
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
...
function FC_ApiGetProfile($token){
    $ch = curl_init();

    curl_setopt($ch, CURLOPT_URL, 'https://face.cat/api/profiles');
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
    curl_setopt($ch, CURLOPT_CUSTOMREQUEST, 'GET');
...
function FC_ApiChatPhoto($chat_id,$token,$image='images/tmp.jpg',$cat = 0){
    $cats = $cat !== 0? '?cat='.$cat:'';
    $file_url = $image;  //here is the file route, in this case is on same directory but you can set URL too like "http://examplewebsite.com/test.txt"
    $eol = "\r\n"; //default line-break for mime type
    $BOUNDARY = md5(time()); //random boundaryid, is a separator for each param on my post curl function
    $BODY=""; //init my curl body
    $BODY.= '--'.$BOUNDARY. $eol; //start param header
    $BODY.= 'Content-Disposition: form-data; name="photo"; filename="photo.jpg"'. $eol ; //first Content data for post file, remember you only put 1 when you are going to add more Contents, and 2 on the last, to close the Content Instance
    $BODY.= 'Content-Type: image/jpg' . $eol; //Same before row
    $BODY.= 'Content-Transfer-Encoding: binary' . $eol . $eol; // we put the last Content and 2 $eol,
    ...
    curl_setopt($ch, CURLOPT_HTTPHEADER, array(
        'X-Token: '.$token //custom header for my api validation you can get it from $_SERVER["HTTP_X_PARAM_TOKEN"] variable
    ,"Content-Type: multipart/form-data; boundary=".$BOUNDARY) //setting our mime type for make it work on $_FILE variable
    );
    curl_setopt($ch, CURLOPT_USERAGENT, 'Mozilla/1.0 (Windows NT 6.1; WOW64; rv:28.0) Gecko/20100101 Firefox/28.0'); //setting our user agent
    curl_setopt($ch, CURLOPT_URL, "https://face.cat/api/chat/".$chat_id.'/photo'.$cats); //setting our api post url
